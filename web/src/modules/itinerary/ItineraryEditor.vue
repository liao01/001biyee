  ...
  <main class="travel-page itinerary-page itinerary-editor-page">
    ...
    <template v-else-if="editor">
      ...
      <div v-if="operationError" class="itinerary-operation-error" role="alert">
        ...
        <button v-if="editor.state.status === 'conflict'" class="travel-secondary-button" type="button" aria-label="重新加载行程" @click="reload">重新加载</button>
      ...
      <section class="itinerary-editor-layout">
        <aside class="itinerary-editor-sidebar">
          <form class="travel-panel itinerary-compact-form" aria-label="基本信息" @submit.prevent="saveOverview">
            ...
            <button class="travel-secondary-button" type="submit">保存基本信息</button>
          </form>
          ...
          <form class="travel-panel itinerary-compact-form" aria-label="目的地" @submit.prevent="saveDestinations">
            ...
            <div v-for="(destination, index) in destinations" :key="destination.id || index" class="itinerary-destination-edit">
              ...
              <div>
                ...
                <button v-if="destinations.length > 1" type="button" :aria-label="`移除目的地 ${destination.name || index + 1}`" @click="destinations.splice(index, 1)">移除</button>
              </div>
            </div>
            <div class="itinerary-inline-actions">
              <button class="itinerary-text-button" type="button" @click="addDestination">添加目的地</button>
              <button class="travel-secondary-button" type="submit">保存目的地</button>
            </div>
          </form>
        ...
        <section class="itinerary-days" aria-label="每日安排">
          ...
          <article v-for="(day, dayIndex) in editor.state.snapshot.days" :key="day.id" class="travel-panel itinerary-day">
            <header class="itinerary-day__header">
              ...
              <button class="itinerary-text-button" type="button" @click="startAdd(day.id)">添加安排</button>
            </header>
            <ol v-if="day.items.length" class="itinerary-items">
              ...
              >
                ...
                <div class="itinerary-item__actions">
                  <button type="button" :disabled="index === 0" :aria-label="`上移 ${item.title}`" :data-item-id="item.id" data-move="up" @click="moveItem(day, index, -1)">上移</button>
                  <button type="button" :disabled="index === day.items.length - 1" :aria-label="`下移 ${item.title}`" :data-item-id="item.id" data-move="down" @click="moveItem(day, index, 1)">下移</button>
                  <button type="button" :aria-label="`编辑 ${item.title}`" @click="startEdit(item)">编辑</button>
                  <button class="is-danger" type="button" :aria-label="`删除 ${item.title}`" @click="removeItem(item)">删除</button>
                </div>
              </li>
            </ol>
          ...
          <form v-if="itemForm.open" class="travel-panel itinerary-item-form" aria-label="安排项" @submit.prevent="saveItem">
            ...
            <div class="itinerary-form__actions"><button class="travel-primary-button" type="submit">保存安排</button></div>
          </form>
        </section>
      </section>
    </template>
...
const saveLabel = computed(() => ({ idle: '尚无更改', saving: '保存中…', saved: '已保存', error: '保存失败', conflict: '存在版本冲突' }[editor.value?.state.status] || ''))
const operationErrorTitle = computed(() => editor.value?.state.status === 'conflict' ? '行程已在其他位置更新' : '这次更改没有保存')
...
const startAdd = (dayId) => Object.assign(itemForm, blankItem(), { open: true, dayId })
const startEdit = (item) => Object.assign(itemForm, blankItem(), { ...item, open: true, startTime: item.startTime || '', endTime: item.endTime || '' })
