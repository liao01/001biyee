...
修订建议始终绑定生成时的行程版本。建议确认会再次读取正式快照，验证负责人、建议状态、期望版本、操作依赖和选择范围，然后把选中操作转换为一个 `ItineraryApplicationService.applyRevision` 命令。行程模块在自己的单一事务内完成全部条目变更和版本递增；任一操作失败则整批回滚。规划模块只有在行程命令成功后才保存确认结果，不直接写 `itinerary`、`itinerary_day` 或 `itinerary_item`。
