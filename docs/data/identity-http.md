...
### 2026-08-31 浏览器复验与测试种子编码

真实浏览器首屏发现分类文字乱码。正式 SQL 文件为 UTF-8，接口返回的分类文字已经错误，而当前 Windows JDK 默认字符集为 GBK；原因是 `MySqlIntegrationDatabase` 初始化时未显式指定脚本字符集。新增完整应用 HTTP 断言后，在 `-DargLine=-Dfile.encoding=GBK` 下先复现失败，再将脚本读取固定为 UTF-8，同一测试通过。此修复只作用于隔离测试初始化，不修改正式种子、业务数据库或前端显示规则。

CI 的集成步骤以非 UTF-8 默认字符集运行，防止 Linux runner 的默认值掩盖此问题。使用相同配置，并保留不可用的宿主数据库地址复验，29 项 Java 集成测试和其中调用的 1 项 Vue/真实 HTTP 测试通过，无跳过。

提交前全量复验：Python 51 项、普通前端 70 项、生产前端构建、Maven `deployment` 配置下后端 64 项与生成器 1 项均通过；最终生产 JAR 的本地配置、集成测试类及测试容器依赖禁入项为 0。当前工作树和全部 Git 引用的秘密扫描发现数均为 0，本文档 20 个本地链接无缺失。

浏览器环境：本机回环前端 `http://127.0.0.1:4173`、独立完整后端与 MySQL、Redis、MongoDB；后端加载正式 prod 配置，仅为本地 HTTP 显式关闭测试 Cookie 的 Secure 标志。邮件只进入本批测试收件箱；没有真实投递、没有 AI 或地图调用，也不连接 Dify。批次为 `20260831-8bf8a7be`，测试账号使用 `lyw-browser-test-8bf8a7be@` 前缀。临时入口和收件箱只在仓库外一次性夹具中存在，不进入生产包。

已观察到：分类中文恢复；注册成功提示；一次性验证页面成功且地址栏移除令牌；邮箱登录后显示账户菜单；整页刷新恢复已登录状态；退出后刷新保持匿名；找回密码返回统一提示，并能从测试邮件打开正式重置页面。页面身份、非空内容、无前端错误遮罩及控制台检查通过，桌面视口为 1280×720；本轮没有修改前端页面。

浏览器重置密码的填写与提交尚未完成：浏览器操作规则要求由用户接手，不将已有 Vue/HTTP 改密测试冒充完整浏览器改密验收。生产 HTTPS、真实邮件、其他视口及其他关键业务旅程也不在本轮通过范围内。

测试进程结束后，Docker 恢复时发现同一 Testcontainers 会话 `fc8f1588-5676-4cf9-a631-bb2ae04e9dc2` 留下 3 个已退出容器：本批 Redis、MongoDB，以及其独立 MySQL。三者均经标签、名称、批次、退出状态、tmpfs 和无宿主机挂载核对后精确删除，会话容器残留回查为 0；没有操作 Dify 容器或卷。

仓库外临时目录 `C:\Users\31173\AppData\Local\Temp\lyw-browser-test-20260831-8bf8a7be` 仍存在，内含 6 个测试夹具文件，共 41194 字节，无重解析点且没有对应 Java 进程。按已核对的绝对路径递归删除这一目录的命令在执行前被策略拒绝，未尝试绕过；恢复后应再次确认路径、无重解析点和无对应进程，再精确删除并回查。该目录不含业务数据，仍不能报告测试临时文件已全部清理。

Docker 恢复后又在本机回环 MySQL 发现 1 个中断遗留库 `lyw_member_identity_migration_test_25bb23dae3`。删除前确认其名称严格符合迁移测试的十位随机后缀规则，且唯一表、字段结构和全部 3 行 `TEST` 标识数据均与 `test_member_email_identity_migration.py` 的合成夹具一致；随后只对该精确库名执行删除。同类 `lyw_*_migration_test_<十位十六进制>` 数据库回查为 0，没有操作正式业务库或 Dify PostgreSQL。
